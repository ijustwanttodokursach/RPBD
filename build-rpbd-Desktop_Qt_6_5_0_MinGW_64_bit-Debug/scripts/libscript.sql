...
CREATE OR REPLACE FUNCTION library.getcopies() returns table
(cid integer, book_name varchar, authors varchar, pub varchar) language plpgsql
as $$
begin
  return query 
    select bc.copy_id, b.title, b.author, p.location from library.books b join library.bookcopy bc on b.book_id = bc.book_id 
join library.publisher
p on p.pub_id = bc.pub_id  WHERE bc.copy_id not in (select iss.book_id from library.issues iss);
    end;
$$;

CREATE OR REPLACE FUNCTION library.getissues() returns table
(bid integer, card_num integer, borrow_date date, deadline date, surname varchar, title varchar, authors varchar, 
 publisher varchar) language plpgsql
as $$
begin
  return query 
    select i.book_id, i.card_num, i.borrow_date, i.deadline, pt.surname, b.title, b.author, pb.location from library.issues i join 
library.bookcopy bc on i.book_id = bc.copy_id join library.books b on b.book_id = bc.book_id join
library.patrons pt on pt.card_num = i.card_num join library.publisher pb on pb.pub_id = bc.pub_id;
    end;
$$;

create or replace function length_phone() returns trigger
language plpgsql
AS $$
DECLARE
len integer;
BEGIN
len = length(new.phone_n);
if len <>11 THEN
RAISE SQLSTATE '20111';
return old;
end if;
return new;
END;
...
create or replace trigger phone_len_tr
before insert on library.patrons
for each row
execute function length_phone();